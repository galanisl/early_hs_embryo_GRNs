...
{
  dpath <- paste0("path/to/GRN/inferences/TFnetMIres_tpm_", ctype, ".csv") #Path for GRN table
  ...
  for (TF in TFs_to_plot)
  {
    ...
    for (Network in Network_types)
    {
      ...
      #There is a more efficient method to do this which is to pre-calculate the Spearman correlation of all the interaction pairs before extracting the networks. But I am too lazy to write a new script.
      ...
      while (Keep_looping)
      {
        ...
        if (nrow(grn_tb_Top_network) > 0)
        {
          ...
          pdf(file=paste(fig_dir, Network, '_of_', TF, '.pdf', sep = ''),width=10,height=10,useDingbats=F);
          {
            ...
            plot.igraph(tf_tf)#, layout = net.graphopt)
            
            legend(-1.4,1.3,legend=levels(as.factor(Legend_levels_V)),pt.cex=Legend_levels_V/2.5, col='black', pt.bg="#EAF6F6", pch = 21, title = 'log2(TPM)', cex = 0.9)
            legend(-1.1,1.3,legend=levels(as.factor(Legend_levels_E)), col='black', lty = 1, lwd=Legend_levels_E, title = 'Interaction score', cex = 0.9)
            legend(-0.7,1.3,legend=levels(factor(c('Activation', 'Repression', 'Insignificant correlation'), levels = c('Activation', 'Repression', 'Insignificant correlation'))), col=c(Activation_edges_color, Repression_edges_color, Insignificant_edges_color), lty = 1, lwd = 2, cex = 0.9)
          }
          dev.off()
        ...
        } else
        {
          ...
          # If network can be constructed before removing TFs from the TF_to_exclude but not after, you will still get the last network plot before no network can be constructed.
          # This is actually a bug but I think this can be a nice feature so I kept it.
          Keep_looping <- F
        }
